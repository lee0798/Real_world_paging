...
void solution(int cache_size, int block_size, int assoc) {
    ...
    block_cnt = calloc(cache_size/block_size, sizeof(int));
...
void update_hit(int block_cnt, int assoc, int user_block, int index){
    if(block_cnt[index][i] < 4){
        block_cnt[index][i]++;
    }
}
int find_block_to_replace(int block_cnt,int assoc){
    int min_block = 0;
    int min_cnt = block_cnt[0];
    int i;
    for (i = 1; i < assoc; i++) {
        if (block_cnt[i] < min_cnt) {
            min_block = i;
        }
    }
    return min_block;
...
void read_op(int addr, int cache_size, int block_size, int assoc){
    ...
    for (i = 0; i < assoc; i++) {
        if (data_cache[index][i].valid && data_cache[index][i].tag == tag) {
            update_hit(block_cnt[index],assoc,i,index);
    ...
    int min_block = find_block_to_replace(block_cnt[index],assoc);
